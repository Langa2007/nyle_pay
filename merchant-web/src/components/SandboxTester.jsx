...
const ENDPOINTS = [
  ...
  { id: 'quote-mpesa-bank', label: 'Quote M-Pesa to Bank', method: 'POST', path: '/api/routes/quote', auth: 'secret', body: { sourceRail: 'MPESA', destinationRail: 'BANK', sourceCurrency: 'KES', destinationCurrency: 'KES', amount: 1500, customerPhone: '254712345678', destination: { bankCode: '01', accountNumber: '1234567890' }, reference: 'TEST-ROUTE-001' }, desc: 'Quote a customer M-Pesa payment settling to a business bank account.' },
  { id: 'quote-crypto-mpesa', label: 'Quote USDT to M-Pesa', method: 'POST', path: '/api/routes/quote', auth: 'secret', body: { sourceRail: 'CRYPTO', sourceAsset: 'USDT', sourceNetwork: 'TRON', destinationRail: 'MPESA', destinationCurrency: 'KES', amount: 25, destination: { phone: '254712345678' }, reference: 'TEST-ROUTE-002' }, desc: 'Quote a crypto intake route that settles to M-Pesa after conversion.' },
  { id: 'execute', label: 'Execute Route', method: 'POST', path: '/api/routes/execute', auth: 'secret', body: { quoteId: 'rtq_test_123', idempotencyKey: 'TEST-ROUTE-001' }, desc: 'Execute a quoted route. Use a quoteId returned by the quote endpoint.' },
